...
def get_roughBPS(time, track_brain):
    ...
    for i in range(len(time)):
        if (waveGamma[i] * track_brain.iloc[i] > 0 
        and waveBeta[i]  * track_brain.iloc[i] > 0 
        and waveAlpha[i] * track_brain.iloc[i] > 0 
        and waveTheta[i] * track_brain.iloc[i] > 0 
        and waveDelta[i] * track_brain.iloc[i] > 0 ):
            temp += 1
